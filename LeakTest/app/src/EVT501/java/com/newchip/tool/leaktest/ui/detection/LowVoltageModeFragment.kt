...
class LowVoltageModeFragment : BaseAppFragment<FragmentLeaktestModeBinding>() {
    ...
    private var upperMaxLimit = 10//超过充气上限10kpa
    private var upperLimit = 5//显示上限
    ...
    private fun initLimit() {
        upperLimit = curPressUnit.convertUnit(5f, ConstantsUtils.KPA).toInt()
        upperMaxLimit = curPressUnit.convertUnit(10f, ConstantsUtils.KPA).toInt()
    ...
    private fun initUI(info: ConfigLeakInfoBean) {
        ...
        mVb.dpvShowProgress.setPressureUnit(
            curPressUnit,
            if (curPressUnit == ConstantsUtils.PA) 60f else 40f
        )
        ...
        mVb.dpvShowPrevalue.setMinAndMaxValue(
            0f,
            info.upperPressureLimit + upperLimit,
            curPressUnit
        )
    ...
    private fun initClick() {
        ...
        mVb.tvSettingClick.setOnClickListener {
            ...
            val action =
                LowVoltageModeFragmentDirections.actionLowVoltageModeFragmentToLowVoltageConfigFragment()
    ...
    private fun setTestStatus(status: Int) {
        val maxSize = mContext.resources.getDimension(R.dimen.sp_19).toInt()
...
//        val limitValue = standardValue * 0.05f
        ...
        if (state == "02") {
            ...
            if (valveState == "80" && currentValue > standardValue - limitValue && currentValue >= lowStabilizationValueLimit) {
                return standardValue
            }
        ...
        if (currentValue > standardValue - limitValue && currentValue >= lowStabilizationValueLimit) {
            return standardValue
        }
        return currentValue
    }
...
//            mVb.tvPressureBackground.text = vm.showValueOne(0f)
    ...
    private fun refreshBtnStatusUI(isOn: Boolean, showProgress: Boolean) {
        ...
        mVb.iconStart.setImageResource(if (!isOn) R.drawable.icon_start_zy_n else R.drawable.icon_start_zy_d)

        mVb.tvStopClick.isEnabled = isOn
        mVb.iconStop.setImageResource(if (isOn) R.drawable.icon_stop_zy_n else R.drawable.icon_stop_zy_d)
