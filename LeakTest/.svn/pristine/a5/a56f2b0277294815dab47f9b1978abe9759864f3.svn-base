...
class MainViewModel(val repo: MainRepo) : BaseViewModel() {
    ...
    fun dealCmdOrder(backOrder: String) {
        ...
        when (receiveData.funcType) {
            ...
            "20" -> {
                val bean = AnalysisTools.analysisClearAdc(receiveData.receiveBuffer)
                LiveEventBus.get(LiveEventBusConstants.CLEAR_ADC_RESULT)
                    .post(bean.result)
            }
    ...
    fun getLowConfigData(): ConfigLeakInfoBean {
    ...
    fun getHighConfigData(): ConfigLeakInfoBean {
        val json = EasyPreferences.instance[ConstantsUtils.HIGH_CONFIG_KEY]
        if (!json.isNullOrEmpty()) {
            return Gson().fromJson(json, ConfigLeakInfoBean::class.java)
        }
        val bean = ConfigLeakInfoBean()
    ...
    fun setLowConfigData(bean: ConfigLeakInfoBean): Boolean {
        return EasyPreferences.instance.putToJson(ConstantsUtils.LEAK_CONFIG_KEY, bean)
    }

    fun setHighConfigData(bean: ConfigLeakInfoBean): Boolean {
        return EasyPreferences.instance.putToJson(ConstantsUtils.HIGH_CONFIG_KEY, bean)
    }
    ...
    fun startDownload(url: String?, version: String, progressCallback: (Int) -> Unit) {
        viewModelScope.launch(Dispatchers.IO) {
            repo.downloadFile(
                ...
                downloadLiveData, progressCallback = progressCallback
            )
        }
    }
    ...
    fun startDownloadApp(url: String?, path: String, progressCallback: (Int) -> Unit) {
        viewModelScope.launch(Dispatchers.IO) {
            repo.downloadFile(url, path, downloadAppLiveData, progressCallback = progressCallback)
        }
    }

//    fun getTimeZone(ipAddress:String){
//        val url = "https://ipapi.co/$ipAddress/timezone"
//        viewModelScope.launch(Dispatchers.IO) {
//            repo.getTimeZone(url, timeZoneLiveData)
//        }
//    }
    ...
    fun startUpdate(mCurrentDevice: IPhysics?, filePath: String?) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            val checkFileInfo =
                FileUtils.readUpdateFile(downloadFile, ConstantsUtils.UPDATE_INFO_NAME_ET500)
