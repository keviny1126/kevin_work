...
//		LogUtil.INSTANCE.e("ykw","------文件校验值----sum："+sum+",sumCheck:"+Integer.toHexString(sum));
    ...
    public static int intPackLength(byte[] str) {
        ...
        int intLength = Long.valueOf(byteStr, 16).intValue();// 十六进制字符串转换为十进制
        return intLength;
    }

    public static long LongPackLength(byte[] str) {
        String byteStr = bytesToHexString(str);
        long intLength = Long.valueOf(byteStr, 16);// 十六进制字符串转换为十进制
        return intLength;
    }
