  ...
  (JNIEnv *env, jclass thiz, jstring path, jint baudrate, jint flags)
{
	...
	/* Opening device */
	{
		...
		LOGD("open() fd = %d errno=%d", fd,errno);
	...
	serInfo.xmit_fifo_size = pagesize;
	LOGD("Configuring xmit_fifo_size %d=",pagesize);
	...
	if (flags1 == -1){
		...
		flags1 &= ~O_NONBLOCK;
		int retVal = fcntl(fd, F_SETFL, flags1);
		ioctl(fd, TCSETS, &options); 
        LOGE("fcntl=%d\n",retVal);     
    }  
  ...
  (JNIEnv *env, jobject thiz)
{
	...
	struct termios options;
	ioctl(descriptor, TCGETS, &options);
	options.c_cflag |= CRTSCTS;
	ioctl(descriptor, TCSETS, &options);
	LOGD("openHardwareFlowControl(fd = %d)", descriptor);
}
/*
 * Class:     cedric_serial_SerialPort
 * Method:    close
  ...
  (JNIEnv *env, jobject thiz)
{
	...
	options.c_cc[VMIN] = 0;
	options.c_cc[VTIME] = 0;
	int retVal = fcntl(descriptor, F_SETFL, flags);
	ioctl(descriptor, TCSETS, &options);
	
	while ((close(descriptor) == -1) && (errno != EBADF)){
		LOGD("repeate close %d errno=%d", descriptor,errno);
	};
	//close(descriptor);
...
JNIEXPORT jint JNICALL Java_com_cnlaunch_physics_serialport_SerialPort_serialPortCheck
  (JNIEnv *env, jclass thiz){
		...
		int fd, rc, ctl;
		int state=0;
		fd = open("/dev/extbd", O_RDONLY);
		LOGD("open() fd = %d errno=%d", fd,errno);
		if(fd < 0) {
			LOGE("Cannot open serial port cts");
			state=1;
		}
		else{
				rc = ioctl(fd, 0x80044700, &ctl);
				if(rc < 0) {
					LOGE("Cannot ioctl serial port cts errno=%d",errno);
					state=1;
				}
				else{
					LOGD("ctl value = %d", ctl);
					if(ctl<=0){
						state=2;
					}
					else{
...
*Signature ()V
*/
JNIEXPORT void JNICALL Java_com_cnlaunch_physics_serialport_SerialPort_tcflush(JNIEnv *env, jobject thiz){
 ...
 * Signature: ()b;
 */
JNIEXPORT jint JNICALL Java_com_cnlaunch_physics_serialport_SerialPort_getReadBufferByte(JNIEnv *env, jobject thiz){
   ...
   jint descriptor = (*env)->GetIntField(env, mFd, descriptorID);
   int bytes=0;   
   ioctl(descriptor, FIONREAD,&bytes);
   LOGD("FIONREAD(bytes = %d)", bytes);
   return bytes;
}